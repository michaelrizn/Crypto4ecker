...
class TestDatabase(unittest.TestCase):
    ...
    def test_insert_and_fetch_market_data(self):
        # Добавляем данные в таблицу market_data.py
        execute_query(
            "INSERT INTO market_data.py (coin_id, price, volume_24h, market_cap) VALUES (?, ?, ?, ?)",
            ("BTC", 50000, 1000000, 900000000)
        )

        # Проверяем, что данные были добавлены корректно
        results = fetch_query("SELECT coin_id, price FROM market_data.py WHERE coin_id=?", ("BTC",))
